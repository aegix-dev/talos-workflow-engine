//! Parallel workflow executor built on `workflow-engine-core` traits.
//!
...
//! `workflow-engine-core`; this crate carries only the scheduling loop,
//! sub-workflow handlers, and the primary engine type.
//!
//! See [`workflow_engine_core`] for the trait surface and
//! `crates/workflow-engine/src/engine.rs` for the executor body.
