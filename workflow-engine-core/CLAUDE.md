...
- **`ExecutionSanitizer` is sub-trait not variant.** The stateless
  `OutputSanitizer` + per-run `ExecutionSanitizer` split mirrors the
  `talos_dlp::ExecutionContext` shape. When adding sanitizer methods,
  decide deliberately which layer they belong on.
...
- **For integration-style testing of traits, use the sibling
  `workflow-engine-test-utils` crate.** When you add a new trait here,
...
- Add a `tokio` dep because "it's convenient" → the logic belongs in
  `workflow-engine` or a consumer crate.
...
- Add a function that does work (not just defines shape) → probably
  belongs in `workflow-engine`. This crate is almost entirely
  declaration.
...
cargo clippy -p workflow-engine-core --all-targets -- -D warnings
cargo doc -p workflow-engine-core --no-deps       # zero warnings
...
cargo clippy -p workflow-engine --lib -- -D warnings
cargo test -p workflow-engine-test-utils
```

A change that compiles here but breaks `workflow-engine` means a trait
