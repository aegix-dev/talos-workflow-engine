...
/// `JsonValue` — no Talos-specific types leak through. That keeps
/// this crate consumable by any engine adopter.
///
...
/// that deviation; the default Talos NATS dispatcher does not honor
/// them per-step because the underlying `PipelineJobRequest` wire
...
pub struct DispatchJob {
    ...
    /// example, the Talos NATS dispatcher's `tokio::time::timeout`)
    /// should add grace on top internally rather than forcing callers
    /// to pre-add it.
    ...
    /// events (e.g. `node_retrying`, `retry_skipped` in the Talos
