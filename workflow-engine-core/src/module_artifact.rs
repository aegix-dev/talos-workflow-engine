...
//! (for example, the Talos `WasmModule` carries a `worker::CapabilityWorld`
//! enum). Adapters flatten those into the `String` fields here.
...
/// filesystem, test map, Talos registry) can populate it trivially.
///
/// # Debug
///
