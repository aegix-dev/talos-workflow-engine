# workflow-engine-nats — Claude guide

NATS-backed [`NodeDispatcher`] and [`JobTransport`] for the
`workflow-engine` crate. Implements the signed job-protocol wire format
([`job_protocol::JobRequest`] + HMAC) that the Talos worker pool speaks.

This file captures the non-obvious rules specific to this crate — the
wire-format contract, retry-loop subtleties, topic routing conventions,
and security invariants. For general engine-extraction rules see
`crates/workflow-engine/CLAUDE.md`.

[`NodeDispatcher`]: workflow_engine_core::NodeDispatcher
[`JobTransport`]: workflow_engine_core::JobTransport
[`job_protocol::JobRequest`]: job_protocol::JobRequest

## Sibling crates

```
workflow-engine-core        types + traits
    │
    └── workflow-engine     executor
             └── workflow-engine-nats   ← you are here
```

This crate depends on `workflow-engine` (for `emit_event_spawn` and the
engine type in `run_with_nats`), `workflow-engine-core` (for trait
objects), and `job-protocol` (for the wire-format structs). **No
controller dep** — that's the whole point.

Consumer dep order:
```
workflow-engine-nats = { path = "..." }   # brings workflow-engine + core transitively
```
...
The Talos worker speaks a specific wire format. Every field of
...
```
talos.jobs[.<user_uuid>][.priority]          single-node
talos.pipeline.jobs[.<user_uuid>][.priority] chain dispatch
```
...
- **`Uuid::nil()` → `None` at the topic layer.** The engine passes
  ...
  `talos.jobs.00000000-...`. See the `.is_nil()` checks in `dispatch`
  and `dispatch_chain`.
...
engine's guarantee that the response actually came from a Talos
worker with the shared key, not an attacker on the NATS wire.
...
```rust
use workflow_engine_nats::{NatsTransport, NatsNodeDispatcher, run_with_nats};
...
### Controller wiring (the Talos-specific glue)

`controller/src/engine/nats_run.rs` owns `build_nats_dispatcher`,
which supplies `HeuristicRetryClassifier` / `RhaiEvaluator` fallbacks
when the engine was built bare. That's Talos convenience and stays
in the controller. **Do not** bring those fallback adapters into
this crate — they're Talos-specific policy.

## Testing

- 0 unit tests in this crate at the time of writing. The engine's
  `cargo test -p workflow-engine --lib` exercises the dispatcher
  indirectly through the 22-test chain/collapse/prefetch harness.
- High-value unit tests to add (none exist yet):
  - Topic routing: nil-UUID → `talos.jobs` (not `talos.jobs.00000000`).
...
Use `workflow-engine-test-utils` to supply a `RetryClassifier` +
...
```
cargo check -p workflow-engine-nats
cargo clippy -p workflow-engine-nats --all-targets -- -D warnings
cargo doc -p workflow-engine-nats --no-deps        # zero warnings
cargo test -p workflow-engine-nats
```

Verify the full workspace (controller is a consumer):
```
cargo check --workspace
cargo test -p workflow-engine --lib
```

## When NOT to modify this crate

- **Talos-specific fallback adapters** (RhaiEvaluator,
  HeuristicRetryClassifier) → belong in the controller's
  `build_nats_dispatcher`.
...
- **New topic convention**: update `get_*_job_topic` + its matching
  ...
  being unset — the default subject must stay `talos.jobs` /
  `talos.pipeline.jobs` for backward compat.
