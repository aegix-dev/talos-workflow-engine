...
//! [`workflow-engine`](../workflow_engine/) crate.
//!
//! Implements the Talos signed-NATS job protocol (see
//! [`job_protocol`]): every dispatched node becomes a
//! `JobRequest` with an HMAC-SHA256 signature over its canonical
//! byte form, sent to a priority-routed NATS subject and matched with a
//! reply `JobResult` that is verified before its output is unwrapped
//! back to the engine.
//!
//! ## Topic routing
//!
//! Single-node jobs publish to `talos.jobs` (or, when
//! `ENABLE_EDGE_ROUTING=true`, `talos.jobs.<user_id>`). Jobs with
//! priority >= 200 are suffixed with `.priority` so workers can
//! subscribe to high-priority work first. Chain dispatches use the
//! parallel `talos.pipeline.jobs[.<user_id>][.priority]` tree.
//!
//! The `Uuid::nil()` user-id sentinel maps back to `None` so "no user
//! context" stays on the tenant-agnostic subject rather than being
//! mis-routed to `talos.jobs.00000000-...`.
//!
//! ## Retry semantics
//!
...
//!   [`workflow_engine_core::RetryClassifier`] and skips retries on
...
//!
//! If you're running the Talos controller + worker, the pairing is
//! mandatory — the worker expects signed `JobRequest`s on the topics
//! above, with the canonical-bytes HMAC the controller ships.
//!
//! If you're embedding [`workflow-engine`](../workflow_engine/) in
//! your own product, you can either:
//!
//! 1. reuse this crate when your workers already speak the Talos job
//!    protocol (easy path); or
...
//!
//! [`EventSink`]: workflow_engine_core::EventSink
